...
def audit_existing(fix: bool = False, verify: bool = False) -> None:
    ...
    logger.info(f"Auditing {len(files)} existing articles (fix={fix}, verify={verify}).")
    ...
    fh.flush()
    counts = {"articles": 0, "stock_images": 0, "bad_trailers": 0, "banned_hits": 0, "covers_rendered": 0, "claims_unverifiable": 0}
    ...
    for path in files:
        ...
        if "images.unsplash.com" in hero:
            ...
            if fix:
                thumb = resolve_thumbnail(
                    {"gameTitle": meta.get("gameTitle", slug), "genre": meta.get("genre", "RPG"), "trailerId": meta.get("trailerId", "")},
                ...
                if thumb.get("url"):
                    ...
                    notes.append(f"  -> replaced with {thumb['source']}: {thumb['url']}")
                    changed = True
        ...
        if hits:
            ...
            notes.append(f"banned words: {[h['word'] for h in hits]} (lines {[h['first_line'] for h in hits]}); manual review recommended")
        ...
        if verify:
            ...
            if not src["ok"] or not src["text"].strip():
                ...
                notes.append(f"source unreachable/unparseable: {src_url} (claims not verified)")
            else:
                audit = audit_claims(body, src["text"])
                cnt = audit["counts"]
                notes.append(f"claim audit vs source: {cnt}")
                if cnt["unsupported"] or cnt["contradicted"]:
                    counts["claims_unverifiable"] += cnt["unsupported"] + cnt["contradicted"]
                    bad = [c for c in audit["claims"] if c.get("verdict") in ("unsupported", "contradicted")]
                    for c in bad[:5]:
                        notes.append(f"  - [{c.get('verdict')}] {str(c.get('claim',''))[:80]}")
...
def _write_article(path: Path, meta: dict[str, Any], body: str) -> None:
    ...
    ordered = {}
    for key in [
        "title", "date", "gameTitle", "developer", "genre", "platforms",
        "releaseWindow", "heroImage", "trailerId", "impactScore", "sourceUrl", "summary",
    ]:
        if key in meta and meta[key] not in (None, ""):
            ordered[key] = meta[key]
    if "specs" in meta and meta["specs"]:
        ordered["specs"] = meta["specs"]
    fm = yaml.safe_dump(ordered, allow_unicode=True, sort_keys=False, default_flow_style=False).strip()
    path.write_text(f"---\n{fm}\n---\n\n{body}", encoding="utf-8")
...
def run_pipeline(provider: str, limit: int, dry_run: bool) -> None:
    ...
    for article in triaged[:limit]:
        ...
        body = generate_editorial_article(facts, article["summary"], provider=provider, source_text=source_text)
        publish_article(facts, body, article["link"], article["impact_score"], source_text=source_text)
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Ingest & triage without saving or spending tokens")
    parser.add_argument("--provider", type=str, default="mock", choices=["mock", "ollama"], help="LLM provider")
    parser.add_argument("--limit", type=int, default=3, help="Max articles to process")
    parser.add_argument("--audit", action="store_true", help="Audit existing src/content/news articles")
    parser.add_argument("--fix", action="store_true", help="With --audit: apply frontmatter fixes (trailers/images)")
    parser.add_argument("--verify", action="store_true", help="With --audit: best-effort claim re-verification vs sourceUrl")
